...
*/				
   ...
   // MissionSettings.DelayTime = 3600;
	...
	void SpawnPatrol(vector pos)//, string loadout = "CaptureFlagLoadout.json") 
	{
		...
		eAIFactionReplicant replicant = new eAIFactionReplicant();
		DayZExpansion game = GetDayZGame().GetExpansionGame();
	...
	void SpawnStuff()
	{
		...
		vector hpos = { Math.RandomFloatInclusive(-50, -30.0), 0, Math.RandomFloatInclusive(-50.0, -30.0) };
		vector ipos = { Math.RandomFloatInclusive(-50, -30.0), 0, Math.RandomFloatInclusive(-50.0, -30.0) };
	...
	void ExtendMission()
	{
		...
		m_MissionMessage2 = "Be aware, the Bandits will likely return soon, and hopefully the pilot will return with the key once the zombies and bandits are gone...";
